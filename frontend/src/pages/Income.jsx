...
// //to helps in converting date to ISO
...
const Income = () => {
    ...
    const fetchOverview = useCallback(
        async (range = timeFrame ?? "monthly") => {
            if (!isAuthenticated()) {
                console.log("Not authenticated, skipping overview fetch");
                return;
            }

            try {
                setLoading(true);
                console.log("Fetching income overview...");
    ...
    useEffect(() => {
        ...
        const token = getToken();
        if (token) {
            console.log("Token found in storage, fetching income data");
            fetchOverview(timeFrame ?? "monthly");
        } else {
            console.log("No token found, skipping income data fetch");
        }
    }, [fetchOverview, timeFrame]);
    ...
    useEffect(() => {
        const token = getToken();
        console.log("Income component mounted, token exists:", !!token);
        if (token) {
    ...
    const handleAddTransaction = useCallback(async () => {
        ...
        if (!isAuthenticated()) {
            alert("Please login to add income");
            window.location.href = '/login';
            return;
        }
