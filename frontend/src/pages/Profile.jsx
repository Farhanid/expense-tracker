...
//                                                 onChange={handleInputChange} className={profileStyles.input} disabled={loading} />
//                                         </div>
...
import { profileStyles } from '../assets/dummyStyles'
import Modal from 'react-modal'
import { Eye, EyeOff, Lock, User, X, ArrowLeft } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import axios from 'axios';
import { toast, ToastContainer } from 'react-toastify'
...
const Profile = ({ user: propUser, onUpdateProfile, onLogout }) => {
    ...
    // Load user data from props first, then from API if needed
    useEffect(() => {
        const loadUserData = async () => {
            // If user is passed from props, use it
            if (propUser && propUser.name) {
                setUser(propUser);
                setTempUser(propUser);
                return;
            }
        ...
        loadUserData();
    }, [propUser, handleApiRequest, onUpdateProfile]);
                        ...
                        <div className={profileStyles.grid}>
                            ...
                            <div className={profileStyles.card}>
                                ...
                                <button
                                    ...
                                    className={`${profileStyles.buttonPrimary} mt-6 w-full bg-red-500 hover:bg-red-600 transition-opacity`}
                                    disabled={loading}
                                >
                                    {loading ? "Processing..." : "Logout"}
                                </button>
