...
import { ArrowDown, BarChart2, ChevronDown, ChevronUp, DollarSign, PieChart , PieChartIcon , PiggyBank, Plus, ShoppingCart, TrendingDown, TrendingUp, Wallet } from 'lucide-react'
...
const API_URL = `${import.meta.env.VITE_URL}/api` || "http://localhost:4000/api"
...
const Dashboard = () => {
  // to get refresh transactions from the outlet context
  const {
  ...
  const displayedExpense = showAllExpense
   ...
   const handleAddTransaction = async () => {
    ...
    try{
      ...
      console.error("Failed to add Transcations:", err?.response || err.message || err)
    ...
    <div className={dashboardStyles.container}>
      ...
      {/*Gauges */}
         ...
         <div className={dashboardStyles.gaugeGrid} >
            {gaugeData.map((gauge) => (
              <GaugeCard key={gauge.name} gauge={gauge} colorInfo={GAUGE_COLORS[gauge.name]} timeFrameLabel={timeFrameRange.label}  />
            ))}
         </div>
...
// // FIXED: Look for "token" not "authToken" - matches what App.js stores
// const getAuthHeader = () => {
...
//             id: item._id || item.id || Date.now() + Math.random(),
//             date: isoDate,
//             description: item.description || item.note || item.title || (typeFromServer === "income" ? item.source || "Income" : item.category || "Expense"),
//             amount: amountNum,
//             type: typeFromServer,
...
//           monthlyIncome: Number(data.monthlyIncome || 0),
//           monthlyExpense: Number(data.monthlyExpense || 0),
//           savings: typeof data.savings !== "undefined" ? Number(data.savings) : Number(data.monthlyIncome || 0) - Number(data.monthlyExpense || 0),
//           savingsRate: typeof data.savingsRate !== "undefined" ? data.savingsRate : null,
//           spendByCategory: data.spendByCategory || {},
//           expenseDistribution: data.expenseDistribution || [],
//           recentTransactions: recent,
//         });
...
//           const monthlyIncome = Number(data.monthlyIncome || 0);
//           const monthlyExpense = Number(data.monthlyExpense || 0);
//           const savings = typeof data.savings !== "undefined" ? Number(data.savings) : monthlyIncome - monthlyExpense;
